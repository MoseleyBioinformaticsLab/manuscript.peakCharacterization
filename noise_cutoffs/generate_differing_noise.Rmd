...
reduce_removing_noise <- function(regions, point_regions, multiplier = 1){
  ...
  chunk_perc = purrr::map_dbl(chunk_indices, function(in_index){
    use_density = sliding_density[seq(in_index, min(in_index + 9999, length(sliding_density)))]
    if (max(use_density) > 0) {
      return(stats::quantile(use_density, 0.99))
    } else {
      return(0)
    }
  })
  
  cutoff_value = median(chunk_perc) * multiplier
