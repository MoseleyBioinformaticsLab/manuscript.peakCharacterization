...
However, the frequency can be calculated by dividing the average M/Z of two adjacent points by their difference (`r figure_count$label_text("mz_frequency_conversion")`A, red points reresenting the average M/Z of two adjacent points, red lines representing the difference between the two adjacent points). 
...
Restricting to those points that fall into a narrow range of frequency differences (0.49 - 0.51), a model of frequency to M/Z can be generated (see Methods), an example of which is shown in `r figure_count$label_text("mz_frequency_conversion")`E. 
...
In the first pass, the peaks who's intensity is correlated with scan are detected.
