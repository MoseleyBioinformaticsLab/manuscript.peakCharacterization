...
hpd_left_panel | hpd_right_panel
```

`r figure_count$label_text("hpd_compare_fsd")`.
Comparison of HPD and high FSD sites in the ECF 2 sample.
Left: Peak plots for various peak processing modalities in a single HPD site. 
*xcalibur*: peaks exported from Thermo Xcalibur after averaging scans.
*scalevel_00*: Scan-level peak characterization without any density based filtering (see noperc_nonorm in Methods).
*scanlevel_99*: Scan-level peak characterization using the default point density filtering (see filtersd in Methods).
*scanlevel_99_lowsd*: Same as *scanlevel_99*, but removing any peaks marked as having a high frequency standard deviation.
*msnbase*: peak centroids generated from *MSnbase*.
Right: Scatterplot of peak counts across all of the HPDs detected in the ECF 2 sample against the Xcalibur peak counts.
