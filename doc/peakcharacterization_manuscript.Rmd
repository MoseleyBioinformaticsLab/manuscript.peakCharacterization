...
::: {custom-style="MDPI_3.1_text"}
To compare p-value changes in a large multi-class sample dataset, we used a much larger set of samples.
...
All samples were characterized using the full scan-centric workflow, as well as peaks generated from the MSnbase R package [@gattoMSnbaseBioconductorPackage2012; @gattoMSnbaseEfficientElegant2020], assigned using SMIRFE, and then peaks matched by shared EMFs across samples (see other methods for a description of peak matching and deciding the most likely EMF).
...
Of the starting 182 samples, five did not finish during scan-centric characterization due to not having any scans left during normalization.
...
Using information-content-informed Kendall-tau correlation, we compared the median correlations of each sample to all others in the same disease class for outliers, and removed `r n_out_vector["cancer"]` cancer and `r n_out_vector["non-cancer"]` samples.

Each samples intensities were normalized by dividing by the median intensities from that sample.
...
The reference peak source is the raw scan-centric p-values.
:::
