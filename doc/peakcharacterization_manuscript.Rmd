...
figure_count$increment("frequency_mode_qcqa")
...
One way is to examine the coefficients of the square-root term for each scan (see term **y** in Equation `r equation_count$label_text("mzfrequency")`) and look for outliers there.
Across the `r length(coefficients_qcqa$n_coef_out)` raw files that completed scan-centric processing, `r sum(coefficients_qcqa$n_coef_out > 0)` had at least one scan removed based on the square-root coefficient.
...
**B**: Single example for sample 199Cpos of the scan square-root terms marked as outliers and removed from further consideration.
:::

One other check we implemented in the conversion to frequency is checking the range of frequency point-point differences allowed for creating the m/z to frequency model, and making sure that they are the same across scans.
The initial set of scans we allowed to use included **all** of the MS1 scans, including the precursor scans for the MS2 scans. 
The samples were acquired as a mix of MS1 only scans for the first 7.5 minutes, and then a mix of MS1 precursor scans followed by MS2 scans for the second 7.5 minutes.
In 20 samples, the MS1 precursor scans have a different mode in the frequency point-point difference than the primary MS1 scans, as shown in `r figure_count$label_text("frequency_mode_qcqa")`.
