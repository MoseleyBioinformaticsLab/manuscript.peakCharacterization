...
**When the intensity is log-transformed, this additive component becomes a dispersive variance component.**[this is true, but I'm not sure it describes these figures**]
...
In the future, the package will be updated check that the resolution is the same across the scans, and produce an informative error for the end user before quitting.
:::
...
In the results presented, we demonstrate superior performance by scan-centric peak characterization in terms of: i) improved QC/QA that removes low quality scans; ii) intensity independent noise reduction that pragmatically eliminates HPD sites; iii) improved peak height relative standard deviations; iv) improved peak intensity ratios that better match natural abundance probabilities; and v) better separation between biological groups.
...
However, correction may not be straight-forward.
...
Therefore, access to the scan level peak intensity information as provided by the methods presented here facilitates the inference of sources and types of error present in the measurements, which we believe have been previously under-described.
:::
...
This should result in a left-censored normal distribution of peak intensities across equivalent scans, which is expected for analytical measurements with detection limits [@flightInformationContentInformedKendalltauCorrelation2022].
